...
from ConfigSpace import Configuration, ConfigurationSpace, Categorical, Integer, Float, EqualsCondition, \
    NotEqualsCondition
...
def get_config_space() -> ConfigurationSpace:
    ...
    model_name = Categorical("model_name", ["MLP", "EarlyJoinSAGE", "LateJoinSAGE", "EarlyJoinResGCN", "LateJoinResGCN"], default="EarlyJoinSAGE")
    embedding_dimensions_log = Integer("op_embed_dim", (1, 10), default=2)
    hidden_dimensions_log = Integer("hidden_dim", (1, 10), default=2)
    mlp_layers = Integer("mlp_layers", (1, 10), default=2)
    num_gnns = Integer("num_gnns", (1, 10), default=2)
    loss = Categorical("loss", ['ListMLELoss:1,MSE:0.02', 'MSE:1', 'ListMLELoss:1'], default="ListMLELoss:1,MSE:0.02")
    clip_norm = Float("clip_norm", (1e-8, 200.0), default=1e-3)
    learning_rate = Float("learning_rate", bounds=(1e-10, 0.1), log=True, default=1e-2)
    batch_size = Integer("batch_size", bounds=(2, 100), default=10)

    cs.add_hyperparameter(model_name)
    cs.add_hyperparameter(embedding_dimensions_log)
    cs.add_hyperparameter(hidden_dimensions_log)
    cs.add_hyperparameter(mlp_layers)
    cs.add_hyperparameter(num_gnns)
    cs.add_hyperparameter(loss)
    cs.add_hyperparameter(clip_norm)
    cs.add_hyperparameter(learning_rate)
    cs.add_hyperparameter(batch_size)

    condition_mlp = EqualsCondition(mlp_layers, model_name, "MLP")
    condition_gnn = NotEqualsCondition(num_gnns, model_name, "MLP")
    cs.add_condition(condition_mlp)
    cs.add_condition(condition_gnn)
    return cs
def convert_smac_configuration_to_train_args(config: Configuration):
    ...
    model_dict = dict()
    model_dict['op_embed_dim'] = 2**(config_dict['op_embed_dim'])
    model_dict['hidden_dim'] = 2**(config_dict['hidden_dim'])

    if model_name == 'MLP':
        model_dict['mlp_layers'] = config_dict['mlp_layers']
    else:
        model_dict['num_gnns'] = config_dict['num_gnns']

    args['model_kwargs_json'] = json.dumps(model_dict)
    args['losses'] = config_dict['loss']
    args['clip_norm'] = config_dict['clip_norm']
    args['learning_rate'] = config_dict['learning_rate']
    args['run_id'] = 1
    args['batch_size'] = config_dict['batch_size']
    args['results_csv'] = ''
    args['out_dir'] = './out/tpugraphs_tiles_beam'
    return train_args.TrainArgs(**args)


def train_model_with_smac_config(config: Configuration, seed) -> float:
    train_args = convert_smac_configuration_to_train_args(config)
    return train_for_smac(args=train_args, seed=seed)


def train_for_smac(args: train_args.TrainArgs, seed):
